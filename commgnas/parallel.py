        ...
        if parallel_operator_num < len(gnn_architecture_list):
            for gnn_group in gnn_index_group_list:
                ...
                for gnn_architecture, parallel_operator in zip(gnn_architecture_list[gnn_group[0]:gnn_group[1]],
                    ...
                    task.append(temp_performance)  #包含feedback和state_dict
                for performance in ray.get(task):
                    result.append(performance)  #添加结果
                # release gpu memory
                #print(f"Updated Best Model: feedback = {best_model['feedback']:.4f}")
...
class GpuEstimation(object):
    ...
    def estimation(self, gnn_architecture):
        ...
        performance = estimator.get_performance()  #包含feedback和state_dict
