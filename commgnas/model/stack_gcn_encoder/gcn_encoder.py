...
class GcnEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.original_feateure_num = original_feature_num   #1433
        self.bias = bias
    ...
    def forward(self, x, edge_index_all):
        ...
        for activation, gnn_layer in zip(self.activation_list, self.gnn_layers_list):
            ...
            output = activation(gnn_layer(output, edge_index_all))   #输入特征矩阵和边
        #     act = activation
        #  out=output
        # if act in ['elu', 'leaky_relu','linear','tanh']:
        #      out=torch.sigmoid(output)
        out=torch.nn.functional.normalize(output, p=2, dim=-1)  #每个节点的特征向量
