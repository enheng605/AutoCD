...
class Search(object):
    ...
    def search_operator(self):  #初始化完开始运行搜索
        ...
        filename = '/history/qwq/' + self.dataname + '/'
        ...
        for iteration in range(self.iteration):
            ...
            messages = [
                {"role": "system",
                 "content": '''Your goal is to select the GNN architecture with the highest loss change value on a given dataset to complete the community detection task. Pay special attention to the content in the special marker #.'''},
                {"role": "user", "content": prompt},
            ]
            completion = client.chat.completions.create(
                model="qwen-plus",  # 模型列表：https://help.aliyun.com/zh/model-studio/getting-started/models
                messages=messages,)
            res_temp=completion.choices[0].message.content
            print(res_temp)
            messages.append(completion)
            messages_history.append(messages)
            with open(filename_message, "w") as f:  # 会自动创建
                json.dump(completion.model_dump_json(), f)
            ...
            with open(root_directory + filename + "iteration_best_" + self.dataname + "_new105_llama.txt", "a+") as f:
                f.write(str(iteration + 1) + "." + str(
                    best_arch) + ": feedback:" + f"{best_feedback:.4f}" + " L_1:" + f"{best_L_1:.4f}" + " L_2:" + f"{best_L_2:.4f}" + " M_1:" + f"{best_M_1:.4f}" + " M_2:" + f"{best_M_2:.4f}" + " S_1:" + f"{best_S_1:.4f}" + " S_2:" + f"{best_S_2:.4f}" + "\n")
