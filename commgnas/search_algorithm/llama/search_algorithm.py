...
class Search(object):
    ...
    def search_operator(self):  #初始化完开始运行搜索
        ...
        filename = '/history/llama/' + self.dataname + '/'
        ...
        for iteration in range(self.iteration):
            ...
            messages = [{'role': 'system', 'content': '''Your goal is to select the GNN architecture with the highest loss change value on a given dataset to complete the community detection task. Pay special attention to the content in the special marker #.'''},
                        {'role': 'user', 'content': prompt}]
            response = dashscope.Generation.call(
                api_key="sk-d466c23679d04b9db4ef63e627edb031",
                model='llama3.3-70b-instruct',
                messages=messages,
                result_format='message',  # set the result to be "message" format.
            )
            res_temp = response.output.choices[0].message.content
            print(res_temp)
            ...
            for list in arch_string:
                ...
                arch_list.append(list_value) #总的
            #result= self.parallel_estimation.estimation(arch_list)   #10个一起评估
            result = self.parallel_estimation.estimation(iter_list)
            #{"feedback": feedback, "L_1": first_loss,  "M_1": first_mod_loss, "S_1": first_sup_loss,"L_2": final_loss, "M_2": final_mod_loss, "S_2": final_sup_loss}
            #per_values = [evaluation["feedback"] for evaluation in result]
            best_arch, best_result = max(zip(iter_list, result), key=lambda x: x[1]["feedback"])  # 最佳架构及对应评估信息
            best_feedback = best_result["feedback"]  # 最大反馈值
            best_L_1 = best_result["L_1"]
            best_M_1 = best_result["M_1"]
            best_S_1 = best_result["S_1"]
            best_L_2 = best_result["L_2"]
            best_M_2 = best_result["M_2"]
            best_S_2 = best_result["S_2"]
            with open(root_directory+filename + "iteration_best_" +self.dataname + "_new105_llama.txt", "a+") as f:
                 f.write(str(iteration+1)+ "."+str(best_arch) + ": feedback:" + f"{best_feedback:.4f}" +" L_1:"+ f"{best_L_1:.4f}" +" L_2:"+ f"{best_L_2:.4f}"+" M_1:"+ f"{best_M_1:.4f}" +" M_2:"+ f"{best_M_2:.4f}" +" S_1:"+ f"{best_S_1:.4f}" +" S_2:"+ f"{best_S_2:.4f}"+"\n")
            #per_list.append(result["feedback"])
            #per_values = per_list[0]
            #存储当前轮的最优架构和性能
            # best_arch = max(zip(iter_list, per_values), key=lambda x: x[1])[0]
            # best_feedback = max(per_values)

            for evaluation in result:
                feedback_list.append(evaluation["feedback"])
            #记录全部的架构和性能
            #feedback_list.append(result["feedback"])
            #feedback_values = [item for sublist in feedback_list for item in sublist]
